...
| `Google Api Error: Invalid request - Precondition check failed.` | Generic Play Store 400 covering several edit-state problems: (1) a previous `supply` run aborted before committing its edit, leaving it `inProgress`; (2) an unintentional draft release exists on internal/beta/production (e.g. the stale 2026-04-24 internal bundles referenced in PR #547); (3) a manual Play Console change is in "Save changes" / "Send for review" state. | **First**: trigger the workflow with `track: validate` (or run `fastlane validate` locally) — this dry-runs `upload_to_play_store` without committing an edit, so the error is reproducible without consuming a versionCode. Then open Play Console → **Releases overview** and discard any unintentional draft releases; check the **Dashboard** for an "unsubmitted changes" banner and send-or-discard. The workflow runs fastlane with `--verbose` + `SUPPLY_VERBOSE=true` and uploads `/tmp/fastlane.log` as a `fastlane-log-<run_id>` artifact on failure — the underlying `googleapis.com/androidpublisher/v3/…` HTTP body in that log contains the specific `FAILED_PRECONDITION` reason. |

The workflow exposes a **`validate`** option in the `track` dispatch input that runs `fastlane validate` (the lane in `android/fastlane/Fastfile`). It calls `upload_to_play_store(validate_only: true, …)` against the internal track, so it exercises auth, AAB integrity, and edit preconditions without committing any changes or consuming a versionCode. Use it after any manual Play Console cleanup to confirm the state is good before a real upload.

Release notes are sourced from `CHANGELOG.md`: the workflow extracts the body of the first `##` section and writes it to `android/fastlane/metadata/android/en-US/changelogs/default.txt` before invoking fastlane. `default.txt` is fastlane-supply's catch-all that applies to any versionCode without a dedicated `<versionCode>.txt`. The legacy `1.txt` is kept for the v1.0 build.
