...
upload to `ANDROID_AUTO_TRACK`, the workflow promotes the _same_ `versionCode`
...
- **Direction matters:** promotion goes _up_ the ladder. Promote
...
`android-publish.yml` run only proves Google _accepted_ the upload. Releases to
...
is to actually _receive_ each build promptly (e.g. to dogfood your own
...
1. Set `ANDROID_AUTO_TRACK = internal` (primary upload goes straight to
   ...
   same build is promoted _up_ to them, where it flows through review in the
   background).
2. **Play Console → Testing → Internal testing → Testers:** add your Google
   account's email, open the internal opt-in URL, and click _Become a tester_.
...
`userFraction`. Use it to confirm _which_ build is on each track and that the
rollout is full.

> **Caveat:** the `status` it prints is the _target_ state you set
