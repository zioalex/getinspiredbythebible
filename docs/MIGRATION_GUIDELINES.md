...
stamping anything: restore a production backup into a throwaway target
(Scenario A or C of [`HOW-TO-BACKUP-RESTORE-DATABASE.md`](HOW-TO-BACKUP-RESTORE-DATABASE.md))
and rehearse the stamp there first — `check` on an unstamped database does not
compare anything, it fails with `Target database is not up to date.`, because
autogenerate refuses to run when the database is not at `head`. So the local
sequence is stamp, then check:
