...
`check` is read-only — it emits no DDL. A clean result means the baseline is
truthful. Drift means prod genuinely differs from `r0001`; reconcile it with a
reviewed revision. **Do not stamp over a mismatch** — stamping tells Alembic
the difference does not exist, and the next autogenerate will try to "fix" it.
