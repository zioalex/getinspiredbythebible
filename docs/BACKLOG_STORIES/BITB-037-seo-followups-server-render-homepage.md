...
canonical + hreflang (incl. `x-default`), and a descriptive home `<title>`
+ `%s · Vox Quieta` template. The shared helper lives at
...
#636 set `twitter.card = "summary"` and OG tags but there is **no
