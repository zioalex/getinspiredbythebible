...
- [x] Per-language coverage matrix committed (canonical / singular / abbreviation /
      numbered-book variants) identifying every gap. See `docs/audits/book-name-coverage.md`,
      generated by `scripts/audit_book_name_coverage.py --check` (also CI-checkable via the
      script's `--check` mode, which asserts no diacritic-fold collisions).
...
- [x] Case- and diacritic-insensitive matching (`normalize_book_name`'s fallback now NFKD-folds
      and strips combining marks before lower-casing, e.g. `"Esaie"`/`"Ésaïe"` both → `Isaiah`),
      with no regressions in `utils/book_names` consumers (verse linking, etc.) — verified 0
      cross-book collisions across all 814 localized/alias source keys.
- [ ] Versification offsets quantified and a documented handling decision (with tests for
      the known offending books).
- [x] Table-driven tests across all 11 languages pass; `search_eval` normalization tests
      still green.
