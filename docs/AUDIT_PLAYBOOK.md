...
| Logic | Copies |
|---|---|
| Verse-reference regex | `frontend/src/lib/versePatterns.ts` · `android/.../ChatMessageItem.kt` · `api/utils/verse_parser.py` — cross-checked by the shared regression corpus `tests/fixtures/verse_reference_corpus.json` (BITB-059 AC#4). BITB-113 unified the literal separator/range/connector grammar fragments genuinely shared across platforms: canonical source `tests/fixtures/verse_grammar.json`, generated into `frontend/src/lib/verseGrammar.generated.ts` **and** `android/.../utils/VerseGrammar.kt` via `scripts/generate_localized_book_map.py --check` (CI-guarded, same generator as the book-name map row below). Reconciled (not generated — Python has no compositional connector-word grammar and its `\d` is already Unicode-aware, see the story's Scope Cut): `api/utils/verse_parser.py`'s separator/range characters, held contradiction-free by `api/tests/test_verse_grammar_parity.py`. Still hand-written per platform, deliberately out of scope: the book-name alternation itself (derived from the already-shared `localized_book_map.json`) and the script-class alternation logic. A server-authoritative alternative now exists: the streaming `completion` event carries `citations` (offset spans into the final answer text — `api/utils/verse_parser.py::extract_citation_spans`, BITB-086). Web now consumes it (BITB-109) — `frontend/src/lib/citationSpans.ts`'s `linkifyWithCitations`, behind `NEXT_PUBLIC_CITATION_SPANS_ENABLED` (default off), falling back per-gap to the regex path for anything a span doesn't validly cover. Android and iOS are not yet clients of it — Android still linkifies via its own regex, and iOS (BITB-087) is planned to inherit the now-web-proven contract instead of a fourth regex grammar. |
| Localized book-name map | Canonical: `tests/fixtures/localized_book_map.json` (BITB-059). Generated: `android/.../utils/LocalizedBookToEnglish.kt` **and** `frontend/src/lib/localizedBookMap.generated.ts`, both via `scripts/generate_localized_book_map.py --check` (CI-guarded). Reconciled (not generated — see rationale in the story): `api/utils/translation_registry.py`, held contradiction-free by `api/tests/test_localized_book_map_registry_parity.py` against a reviewed, pinned gap allowlist (`tests/fixtures/localized_book_map_registry_gaps.json`). Now generated (see the row above): BITB-113 brought the literal separator/range/connector grammar fragments into this same generator (`verse_grammar.json` -> `verseGrammar.generated.ts` / `VerseGrammar.kt`). Still hand-written per platform, deliberately out of scope: the book-name alternation logic itself (which literal names to list, derived from this map) and the script-class alternation logic. |
| Traditional→Simplified char map | Canonical: `tests/fixtures/t2s_char_map.json` (BITB-025/BITB-110). Hand-ported copies (not generated — BITB-108/113's generator does not yet cover script-class alternations): `api/utils/chinese_script.py`, `frontend/src/lib/chineseScript.ts`, `android/.../utils/ChineseScript.kt` — each verified against the fixture by its own parity test (`api/tests/test_chinese_script.py`, `frontend/src/lib/chineseScript.test.ts`, `android/.../utils/ChineseScriptTest.kt`). |
