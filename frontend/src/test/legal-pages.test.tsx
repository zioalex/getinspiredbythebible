...
vi.mock('next-intl/server', () => ({
  getTranslations: vi.fn().mockImplementation(({ namespace }) => {
    const ns = enMessages[namespace as keyof typeof enMessages] as Record<string, string>;
...
const nonEnglishLocales = ['ar', 'de', 'es', 'fr', 'hi', 'it', 'ko', 'pt', 'ru', 'zh'];

describe('Localized legal documents', () => {
  for (const locale of nonEnglishLocales) {
    it(`privacy-policy.${locale}.md exists`, () => {
      expect(existsSync(join(legalDir, `privacy-policy.${locale}.md`))).toBe(true);
    });

    it(`terms-of-service.${locale}.md exists`, () => {
      expect(existsSync(join(legalDir, `terms-of-service.${locale}.md`))).toBe(true);
    });
  }
});
