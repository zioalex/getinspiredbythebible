...
describe("RootPage", () => {
  ...
  it("redirects to /de for German browser", async () => {
    mockHeaders.mockResolvedValue(
      createMockHeaders("de-DE,de;q=0.9,en-US;q=0.8,en;q=0.7"),
    );
    await RootPage();
  ...
  it("redirects to /it for Italian browser", async () => {
    mockHeaders.mockResolvedValue(
      createMockHeaders("it-IT,it;q=0.8,en-US;q=0.5,en;q=0.3"),
    );
    await RootPage();
  ...
  it("picks best supported locale when preferred is unsupported", async () => {
    mockHeaders.mockResolvedValue(
      createMockHeaders("fr;q=1.0,it;q=0.8,en;q=0.5"),
    );
    await RootPage();
    expect(mockRedirect).toHaveBeenCalledWith("/it");
  });
