...
describe('Home page responsive layout', () => {
  ...
  describe('responsive main container', () => {
    ...
    it('renders messages area with responsive padding', () => {
      renderWithIntl(<Home />);
      const messagesArea = document.querySelector('[class*="overflow-y-auto"][class*="px-3"]');
  ...
  describe('mobile FAB (Floating Action Button)', () => {
    it('does not show FAB when there are no verses', () => {
      renderWithIntl(<Home />);
      expect(screen.queryByLabelText('Show scripture references')).not.toBeInTheDocument();
    });
  ...
  describe('Suggested prompts', () => {
    it('submits a message when a suggested prompt is clicked', async () => {
      ...
      const prompts = screen
        .getAllByRole('button')
        .filter(b => !b.querySelector('svg') && b.className.includes('text-left'));
      ...
      await waitFor(() => {
        expect(screen.getByText('Response to suggested prompt')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('mobile slide-over panel', () => {
    ...
    it('closes panel when close button is clicked', async () => {
      ...
      expect(screen.getAllByText('Scripture References').length).toBe(beforeCount + 1);
      ...
      expect(screen.getAllByText('Scripture References').length).toBe(beforeCount);
    });
    ...
    it('panel has lg:hidden class so it only shows on mobile', async () => {
      ...
      const panel = document.querySelector('[class*="lg:hidden"][class*="fixed"]');
    ...
    it('resets mobile panel state on New Chat', async () => {
      ...
      const beforeResetCount = screen.getAllByText('Scripture References').length;
  ...
  describe('Turnstile security checks', () => {
    ...
    it('disables suggested prompts before /config has resolved', () => {
      ...
      expect(screen.getByText('Preparing secure connection...')).toBeInTheDocument();
    });
...
describe('smart auto-scroll', () => {
  ...
  const getMessagesContainer = () =>
    document.querySelector('[class*="overflow-y-auto"][class*="px-3"]') as HTMLElement;
  ...
  it('sending a new message resets auto-scroll to follow new content', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('Test response').length).toBeGreaterThan(1));
...
describe('language-switch suggestion', () => {
  ...
  it('does not show the banner when language_suggestion is null', async () => {
    ...
    await waitFor(() => expect(screen.getByText('A response')).toBeInTheDocument());
