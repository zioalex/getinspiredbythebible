...
import {
  CONNECTOR_WORDS,
  CONNECTOR_REPEAT_MAX,
  CHAPTER_VERSE_SEPARATORS,
  RANGE_SEPARATORS,
  NON_ASCII_DIGIT_RANGES,
  CJK_BRACKET_PAIRS,
} from "./verseGrammar.generated";

// ---------------------------------------------------------------------------
// Grammar fragments composed from the generated constants (BITB-113)
// ---------------------------------------------------------------------------
//
// tests/fixtures/verse_grammar.json is the single source of truth for these literal
// fragments; verseGrammar.generated.ts (generated by
// scripts/generate_localized_book_map.py) exports the raw pieces, and this module does
// its own escaping/composition into regex source fragments — never a compiled RegExp is
// exported from the generated file.

/** Connector words alternation, e.g. "of|dei|des|der|van|de|af|dos|da|del|के|ال". */
const CONNECTOR_ALTERNATION = CONNECTOR_WORDS.join("|");

/** Chapter/verse separator character-class contents, e.g. ":,". */
const CV_SEPARATOR_CLASS = CHAPTER_VERSE_SEPARATORS.join("");

/** Range-separator character-class contents, e.g. "-–" (hyphen must stay first so it is
 * treated as a literal, not a range operator, when embedded in a `[...]` class). */
...
/** Closing brackets, paired with OPEN_BRACKETS above. */
const CLOSE_BRACKETS = CJK_BRACKET_PAIRS.map((b) => b.close).join("");
...
function buildPatternSource(): string {
  ...
  const digitClass = `[\\d${NON_ASCII_DIGIT_CLASS}]`;

  _cachedPatternSource =
    `(?:(?<!\\p{L})|(?<=\\p{Script=Han})|(?<=\\p{Script=Hangul})|(?<=\\p{Script=Devanagari})|(?<=[${OPEN_BRACKETS}]))(${multiWordPart}` +
    `[\\p{L}\\p{M}]{2,}(?:\\s+(?:${CONNECTOR_ALTERNATION})\\s+[\\p{L}\\p{M}]+){1,${CONNECTOR_REPEAT_MAX}}` +
    ...
    `${hangulPart}` +
    `${devanagariPart}` +
    `(?:(?![\\p{Script=Han}\\p{Script=Hangul}\\p{Script=Devanagari}])[\\p{L}\\p{M}]){2,})[${CLOSE_BRACKETS}]?(?:(?<=[\\p{Script=Han}])\\s*|(?<=[\\p{Script=Hangul}])\\s*|(?<=[\\p{Script=Devanagari}])\\s*|(?<=[${CLOSE_BRACKETS}])\\s*|\\s+)(${digitClass}+)[${CV_SEPARATOR_CLASS}](${digitClass}+)(?:[${RANGE_SEPARATOR_CLASS}](${digitClass}+))?`;
