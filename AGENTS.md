...
**Never push to a branch that has a closed or merged PR.** If the PR for a branch has been closed or merged, that branch is done. Create a new branch (from `main`) for any further work, even if it's related. Pushing to a dead branch causes confusion and orphaned commits.
