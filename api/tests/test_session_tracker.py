...
@pytest.mark.asyncio
async def test_track_session_normalizes_blank_user_agent_to_none():
    db = MagicMock()
    db.execute = AsyncMock()

    await track_session(db, session_token="tok-blank", user_agent=" \t ")

    _, params = db.execute.call_args.args
    assert params["ua"] is None
    assert params["mobile"] is None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("VoxQuieta/1.8.0 (Android 14)", True),
        # OkHttp is also used by non-Android JVM clients and is not a mobile signal.
        ("okhttp/4.12.0", False),
